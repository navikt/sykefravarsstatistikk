...
const mockSammenligningNotAuthorized = () => fetchMock.get('express:/sykefravarsstatistikk/api/:orgnr/sammenligning', {status: 403, body: "You are not authorized to view this resource"});
...
} else if (process.env.REACT_APP_MOCK_403) {
    mockSammenligningNotAuthorized();
    mockMinSideArbeidsgiver().spy();
