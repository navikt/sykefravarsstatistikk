...
it('Should succeed and parse correctly', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAggregertStatistikk(happyFetcher), { wrapper });
...
it('Should return IkkeInnlogget if status code is 401', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAggregertStatistikk(notLoggedInFetcher), { wrapper });
...
it('Should return IngenTilgang if status code is 403', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAggregertStatistikk(notAuthorizedFetcher), { wrapper });
...
it('Should fail if status code is not 2xx', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAggregertStatistikk(invalidStatusFetcher), { wrapper });
...
it('Should fail if data fails validation', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAggregertStatistikk(invalidDataFetcher), { wrapper });
...
it('Should fail and return thrown error', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAggregertStatistikk(explodingFetcher), { wrapper });
                ...
                'prosentSiste4KvartalerTotalt':
                    {
                        ...
                        'kvartalerIBeregningen': [{ 'årstall': 2021, 'kvartal': 3 }, {
                            'årstall': 2021,
                            'kvartal': 4,
                        }, { 'årstall': 2022, 'kvartal': 1 }, { 'årstall': 2022, 'kvartal': 2 }],
                    },
        },
    ],
    [ 'SEKTOR', {} ],
